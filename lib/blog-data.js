...
export async function listBlogs({ publicOnly = false, status, category, search, language, page = 1, limit = 10 } = {}) {
  const supabase = getSupabaseAdmin();
...
export async function getBlogByIdOrSlug(idOrSlug, publicOnly = false, language = null) {
  const supabase = getSupabaseAdmin();
